...
export const articleController = {
  ...
  async getArticles(req: Request, res: Response) {
    ...
    // 2. 파라미터 파싱
    const keyword =
      typeof keywordName === "string" && keywordName.trim() !== ""
        ? keywordName
        : undefined;
  ...
  async readArticleDetail(req: Request, res: Response) {
    ...
    if (isNaN(topicIdNum) || isNaN(articleId) || topicIdNum <= 0) {
      return res
        .status(400)
        .json({ message: "유효하지 않은 토픽 ID 또는 기사 ID입니다." });
    }
  ...
  async deleteArticle(req: Request, res: Response) {
    ...
    try {
      const rowCount = await ArticleService.deleteArticle(
        topicIdNum,
        articleId
      );
...
export const getPersonalizedFeed = async (req: Request, res: Response) => {
  ...
  if (!userId) {
    // authMiddleware를 통과하지 못했을 경우 (401 에러가 이미 발생했어야 함)
    return res.status(403).json({ message: "User ID not found." });
  }

  try {
    const articles = await getPersonalizedArticles(userId);
